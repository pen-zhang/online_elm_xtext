/**
 * generated by Xtext 2.24.0
 */
package org.xtext.online_elm.onlineElm;

import java.util.Arrays;
import java.util.Collections;
import java.util.List;

import org.eclipse.emf.common.util.Enumerator;

/**
 * <!-- begin-user-doc -->
 * A representation of the literals of the enumeration '<em><b>COMPARISON</b></em>',
 * and utility methods for working with them.
 * <!-- end-user-doc -->
 * @see org.xtext.online_elm.onlineElm.OnlineElmPackage#getCOMPARISON()
 * @model
 * @generated
 */
public enum COMPARISON implements Enumerator
{
  /**
   * The '<em><b>Bigger</b></em>' literal object.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #BIGGER_VALUE
   * @generated
   * @ordered
   */
  BIGGER(0, "bigger", ">"),

  /**
   * The '<em><b>No Less Than</b></em>' literal object.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #NO_LESS_THAN_VALUE
   * @generated
   * @ordered
   */
  NO_LESS_THAN(1, "noLessThan", ">="),

  /**
   * The '<em><b>Smaller</b></em>' literal object.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #SMALLER_VALUE
   * @generated
   * @ordered
   */
  SMALLER(2, "smaller", "<"),

  /**
   * The '<em><b>No Bigger Than</b></em>' literal object.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #NO_BIGGER_THAN_VALUE
   * @generated
   * @ordered
   */
  NO_BIGGER_THAN(3, "noBiggerThan", "<="),

  /**
   * The '<em><b>Equal</b></em>' literal object.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #EQUAL_VALUE
   * @generated
   * @ordered
   */
  EQUAL(4, "equal", "==");

  /**
   * The '<em><b>Bigger</b></em>' literal value.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #BIGGER
   * @model name="bigger" literal="&gt;"
   * @generated
   * @ordered
   */
  public static final int BIGGER_VALUE = 0;

  /**
   * The '<em><b>No Less Than</b></em>' literal value.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #NO_LESS_THAN
   * @model name="noLessThan" literal="&gt;="
   * @generated
   * @ordered
   */
  public static final int NO_LESS_THAN_VALUE = 1;

  /**
   * The '<em><b>Smaller</b></em>' literal value.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #SMALLER
   * @model name="smaller" literal="&lt;"
   * @generated
   * @ordered
   */
  public static final int SMALLER_VALUE = 2;

  /**
   * The '<em><b>No Bigger Than</b></em>' literal value.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #NO_BIGGER_THAN
   * @model name="noBiggerThan" literal="&lt;="
   * @generated
   * @ordered
   */
  public static final int NO_BIGGER_THAN_VALUE = 3;

  /**
   * The '<em><b>Equal</b></em>' literal value.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #EQUAL
   * @model name="equal" literal="=="
   * @generated
   * @ordered
   */
  public static final int EQUAL_VALUE = 4;

  /**
   * An array of all the '<em><b>COMPARISON</b></em>' enumerators.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private static final COMPARISON[] VALUES_ARRAY =
    new COMPARISON[]
    {
      BIGGER,
      NO_LESS_THAN,
      SMALLER,
      NO_BIGGER_THAN,
      EQUAL,
    };

  /**
   * A public read-only list of all the '<em><b>COMPARISON</b></em>' enumerators.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  public static final List<COMPARISON> VALUES = Collections.unmodifiableList(Arrays.asList(VALUES_ARRAY));

  /**
   * Returns the '<em><b>COMPARISON</b></em>' literal with the specified literal value.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param literal the literal.
   * @return the matching enumerator or <code>null</code>.
   * @generated
   */
  public static COMPARISON get(String literal)
  {
    for (int i = 0; i < VALUES_ARRAY.length; ++i)
    {
      COMPARISON result = VALUES_ARRAY[i];
      if (result.toString().equals(literal))
      {
        return result;
      }
    }
    return null;
  }

  /**
   * Returns the '<em><b>COMPARISON</b></em>' literal with the specified name.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param name the name.
   * @return the matching enumerator or <code>null</code>.
   * @generated
   */
  public static COMPARISON getByName(String name)
  {
    for (int i = 0; i < VALUES_ARRAY.length; ++i)
    {
      COMPARISON result = VALUES_ARRAY[i];
      if (result.getName().equals(name))
      {
        return result;
      }
    }
    return null;
  }

  /**
   * Returns the '<em><b>COMPARISON</b></em>' literal with the specified integer value.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the integer value.
   * @return the matching enumerator or <code>null</code>.
   * @generated
   */
  public static COMPARISON get(int value)
  {
    switch (value)
    {
      case BIGGER_VALUE: return BIGGER;
      case NO_LESS_THAN_VALUE: return NO_LESS_THAN;
      case SMALLER_VALUE: return SMALLER;
      case NO_BIGGER_THAN_VALUE: return NO_BIGGER_THAN;
      case EQUAL_VALUE: return EQUAL;
    }
    return null;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private final int value;

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private final String name;

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private final String literal;

  /**
   * Only this class can construct instances.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private COMPARISON(int value, String name, String literal)
  {
    this.value = value;
    this.name = name;
    this.literal = literal;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public int getValue()
  {
    return value;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public String getName()
  {
    return name;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public String getLiteral()
  {
    return literal;
  }

  /**
   * Returns the literal value of the enumerator, which is its string representation.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public String toString()
  {
    return literal;
  }
  
} //COMPARISON
